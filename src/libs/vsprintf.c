...
#define PLUS    4           // 显示 + 号
#define SPACE   8           // 如果是+，就置空格
#define LEFT    16          // 左调整
#define SPECIAL 32          // 0x 0b
...
static char *number2str(char *str, unsigned long num, int base, int size, int precision, int flags) {
    char c, sign, tmp[36];
    ...
    // 如果进制基数小于2或大于36 则退出处理
    if (base < 2 && base > 36) {
        return 0;
    }

    // 如果 flags 指出要填充零， 则置字符变量为 '0' 否则为空格字符
    c = (flags & ZEROPAD) ? '0' : ' ';

    // 如果 flags 指出是带符号数值并且数值 num 小于 0，则置符号变量 sign=符号，并使 num 取绝对值
    if (flags & SIGN && num < 0) {
        sign = '-';
        num = -num;
    } else {
    ...
    // 若 flags 是特殊转换，则对于十六进制宽度再减少2位 (0x)
    if (flags & SPECIAL) {
        if (base == 16) {
            size -= 2;
        } 
    ...
    if (num == 0) {
        ...
        while (num != 0) {
            index = num % base;
            num /= base;
            tmp[i++] = digits_str[index];
        }
    }

    // 若数值字符个数大于精度值，则精度值扩展位数字个数值
    if (i > precision) {
        precision = i;
    }
    // 宽度值 size 减去用于存放数字字符的个数
    size -= precision;

    // 形成所需要的转换结果，并暂时放在字符串 str 中

    // 若 flags 中没有填零(ZEROPAD) 和 左对齐 (LEFT) 标志
    // 则在 str 中首先放剩余宽度值的空格数
...
int vsprintf(char *buf, const char *fmt, va_list args)
{
    ...
    repeat:
        ...
        case 'o':
            str = number2str(str, va_arg(args, unsigned long), 8,
        ...
        case 'x':
            ...
            flags |= SMALL;
        case 'X':
            str = number2str(str, va_arg(args, unsigned long), 16,
                         field_width, precision, flags);
            break;
        ...
        // 'u'代表无符号整数
        case 'u':
            str = number2str(str, va_arg(args, unsigned long), 10,
                         field_width, precision, flags);
            break;
